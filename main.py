...
def main_logic():
    file_names , urls = get_pdf_files_from_bucket()
    table_formats = []
    for file_name in file_names:
        print(f"we are triggering main logic for {file_name}")
        header_df, lines_df = bucket(file_name)
        print(header_df.head())
        header_csv_file = file_name + '_headers_' + '.csv'
        lines_csv_file = file_name + '_lines_' + '.csv'
